...
sanity_check() {
    # only meant to be run on Ubuntu 22.04 server, not in local dev
    if ! which lsb_release &>/dev/null || [[ "$(lsb_release -is)" != "Ubuntu" ]]; then
        echo "ERROR: this script is meant to be run on the datasette-private server in prod"
        exit 1
    fi
}
